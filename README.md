...
**State Pattern**  
- I used State pattern for different bosses behavior, becoming easier to develop them. The idea for this patten is the current state call the next state depending the condition for calling the next state, usually when the Animation/Attack ended and the boss call the next attack.
- This approach have 2 Advanages:
  ...
  - **Can reuse others states**: The exploding state is an example, every boss use the same explosing animation because of this pattern;
...
- The final boss, for being the most complex doesn't use the interface the same way as the other bosses, mainly beacuse each state is an standalone scene. 
...
- But, this isn't an accurate implementation of Singleton because the classes it doesn't have an private constructor. The reason why is for the fact that godot only support partial classes for node objects, and private constructors doesn't work. The workaround I made is setting the private static property on the `_Ready()` function. While working for most of the time, it might happen to have some problems when the game is loading for the first time. So I made de implementation carefully.
...
- With theses dummies objects is possible to configurate especifics settings for each enemy without dependencies, such as position, speed and the projectile type.
